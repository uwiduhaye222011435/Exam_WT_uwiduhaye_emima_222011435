...
<input type="sid" id="sid" name="sid" required><br><br>
<label for="cid">course_id:</label>
<input type="number" id="cid" name="cid" required><br><br>
<label for="enr">enrollment_date:</label>
<input type="date" id="enr" name="enr" required><br><br>
...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] =="POST") {
       ...
       // Preparing SQL query with prepared statement
        $stmt = $connection->prepare("INSERT INTO enrollments (student_id, course_id, enrollment_date) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $sid, $cid, $enr);
     
        if ($stmt->execute()) {
            // Redirecting to login page on successful registration
            // header("Location: login.html");
            echo "Data inserted successfully. <a href='Enrollements.php'>OK</a>";
            exit();
        } else {
...
<body>
    <center><h2>Table of Enrollement</h2></center>
    <table border="5">
        <tr>
            <th>enrollment_id</th>
            <th>student_id</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_enrollements.php?enrollment_id=$enrollment_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_enrollements.php?enrollment_id=$enrollment_id'>Update</a></td> 
                </tr>";
            }
        } else {
