...
<label for="mdame">module_name:</label>
<input type="name" id="mdame" name="mdame" required><br><br>


<label for="star">Start_date:</label>
<input type="date" id="start_date" name="star" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] =="POST") {
    ...
    $cid= $_POST['cid'];
   // Preparing SQL query
    $sql = "INSERT INTO modules(module_name,start_date,end_date,course_id)VALUES('$mdame','$start','$end','$cid')";
 
   
    if ($connection->query($sql) === TRUE) {
        // Redirecting to login page on successful registration
       // header("Location: login.html");
        echo "data inserted well.<a href='modules.php'>OK</a>";
        exit();
    } else {
...
<body>
    <center><h2>Table of module</h2></center>
    <table border="5">
        <tr>
            <th> module_id</th>
            <th>module_name</th>
    ...
    include('connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $module_id = $row['module_id']; // Fetch the  module_id
                echo "<tr>
                    <td>" . $row['module_id']."</td>
