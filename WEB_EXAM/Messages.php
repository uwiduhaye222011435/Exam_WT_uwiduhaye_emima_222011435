...
<label for="sent_date">sent_date:</label>
<input type="date" id="sent_date" name="sent_date" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] =="POST") {
   ...
   // Preparing SQL query
    $sql = "INSERT INTO messages (message_text,sent_date) VALUES ('$message_text','$sent_date')";
 
   
    if ($connection->query($sql) === TRUE) {
        // Redirecting to login page on successful registration
       // header("Location: login.html");
        echo "data inserted well.<a href='messages.php'>OK</a>";
        exit();
    } else {
    ...
    include('connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $message_id = $row['message_id']; // Fetch the  module_id
                echo "<tr>
                    <td>" . $row['message_id'] . "</td>
                     <td>" . $row['message_text'] . "</td>
                     <td>" . $row['sent_date'] . "</td>
            
                    <td><a style='padding:4px' href='delete.messages.php?message_id=$message_id'>Delete</a></td> 
